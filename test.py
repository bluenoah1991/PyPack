...
def test(socket, address):
    PyPack.commit('123', "First bootstrap message!", 0)
    PyPack.commit('123', "Second bootstrap message! (Qos 1)", 1)
    PyPack.commit('123', "Third bootstrap message! (Qos 2)", 2)
    PyPack.hold('123', socket.makefile(), recieved_msg)
